...
def parse() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rejection via Distributions (train base classifier)"
    )
...
if __name__ == "__main__":
    ...
    logging_file = pathlib.Path(
        f"{args.log_dir}/{args.dataset}_{args.noise_rate}/{args.fold_idx}/base.log"
    )
    ...
    if clf.save_path(args.model_folder, save_sub).is_file():
        raise SystemExit
    ...
    optimizer = torch.optim.Adam(
        clf.parameters(), lr=args.lr, weight_decay=1e-4
    )

    for epoch in trange(1, args.epochs + 1):
        classifier.train_cpe_network(
            clf, optimizer, device, clf_train_loader, epoch
        )
        utils.report_performance_cpe(clf, acceptor, 0, device, test_loader)

    # Train Temperature
    classifier.train_cpe_temperature(clf, device, clf_val_loader)
    utils.report_performance_cpe(clf, acceptor, 0, device, test_loader)

    clf.save(args.model_folder, save_sub)
